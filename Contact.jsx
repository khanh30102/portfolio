...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!serviceID || !templateID || !publicKey) {
      console.error("LỖI: Thiếu cấu hình EmailJS. Hãy kiểm tra file .env đã có đủ 3 biến VITE_ chưa.");
      setStatus('error');
      return;
    }
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg">
          <form ref={form} onSubmit={handleSubmit} className="space-y-6">
            ...
            {status === 'error' && <p className="text-red-500 text-center mt-4">Có lỗi xảy ra, vui lòng thử lại.</p>}
          </form>
        </motion.div>
